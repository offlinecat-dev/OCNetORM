...
```typescript
import { RelationMetadata, RelationType, MetadataStorage } from '@offlinecat/ocorm'
...
// 注册关系
MetadataStorage.getInstance().registerRelation('User', userToProfile)
```
...
```typescript
// User -> Profile
MetadataStorage.getInstance().registerRelation('User', new RelationMetadata(
  RelationType.ONE_TO_ONE,
  'User',
  'Profile',
  'profile',
...
// Profile -> User（反向）
MetadataStorage.getInstance().registerRelation('Profile', new RelationMetadata(
  RelationType.ONE_TO_ONE,
  'Profile',
  'User',
  'user',
...
// 需要时再加载关联（显式触发）
if (user !== null) {
  const profile = await user.loadRelatedSingle('profile')
}
```
...
```typescript
import { CascadeOptions } from '@offlinecat/ocorm'
...
async function updateUserProfile(userId: number, bio: string, avatar: string) {
  ...
  const profile = await new QueryExecutor(qb).getOne()
