...
使用 `withLazy` 方法标记关联为延迟加载，关联数据在首次访问时才会加载：
...
// 此时 posts 尚未加载
// 首次访问时会触发加载（需要 RelationLoader 支持）
```
...
```typescript
import { RelationLoader, EntityData } from '@aspect/ocorm'
...
```typescript
import { RelationLoadResult } from '@aspect/ocorm'
...
```typescript
import { ParallelRelationLoadResult } from '@aspect/ocorm'
...
for (const user of users) {
  const posts = user.getRelations('posts')  // 已加载，无额外查询
}
```
