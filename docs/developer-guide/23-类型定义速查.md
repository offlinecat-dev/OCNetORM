...
| 级别 | 脏读 | 不可重复读 | 幻读 | 说明 |
|------|------|------------|------|------|
| `READ_UNCOMMITTED` | ✓ | ✓ | ✓ | 已支持 |
| `READ_COMMITTED` | ✗ | ✓ | ✓ | 已支持（默认） |
...
class TransactionOptions {
  ...
  static serializable(): TransactionOptions  // 当前实现在创建时直接抛错
}
```

> 说明：`serializable()` 为 fail-fast 入口；若通过 `fromConfig` 手动设置 `SERIALIZABLE/REPEATABLE_READ`，会在执行阶段抛错。
