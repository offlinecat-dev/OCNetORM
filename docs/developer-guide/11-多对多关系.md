...
```typescript
import { ManyToManyMetadata, MetadataStorage } from '@offlinecat/ocorm'
...
使用 `SchemaBuilder` 自动建表时，多对多关系的中间表默认包含两个 NOT NULL 列，并通过唯一约束防止重复关联（当前实现不会自动创建外键约束）：
...
const post = await new QueryExecutor(qb).getOne()

if (post !== null) {
  // 获取关联的标签列表
  const tags = post.getRelatedArray('tags')  // 返回 EntityData[]
...
const tag = await new QueryExecutor(qb).getOne()

if (tag !== null) {
  const posts = tag.getRelatedArray('posts')
...
async function getPostsByTag(tagName: string): Promise<EntityData[]> {
  ...
  const tag = await new QueryExecutor(tagQb).getOne()
  
  if (tag === null) {
    return []
  }
  
  return tag.getRelatedArray('posts')
}
...
async function getPopularTags(limit: number): Promise<EntityData[]> {
  ...
  const sortedTags = tags.sort((a, b) => {
    const countA = a.getRelatedArray('posts').length
    const countB = b.getRelatedArray('posts').length
...
1. **中间表管理**：如果使用 `SchemaBuilder.createAllTablesWithManager()`（默认 `includeJoinTables=true`），OCORM 会根据 `ManyToManyMetadata` 自动创建中间表；否则需要自行创建或通过迁移处理
2. **外键顺序**：定义反向关系时，注意 `joinSourceKey` 和 `joinTargetKey` 的顺序需要对调
3. **级联删除**：如你在中间表上自行创建外键约束，建议设置 `ON DELETE CASCADE`，确保删除实体时自动清理关联
