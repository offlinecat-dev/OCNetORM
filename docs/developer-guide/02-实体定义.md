...
```typescript
import { Table, registerEntity } from '@aspect/ocorm'
...
```typescript
import { registerEntity, createEntityOptions } from '@aspect/ocorm'
...
```typescript
import { Table, Column, PrimaryKey, CreatedAt, UpdatedAt, SoftDelete, ColumnType } from '@aspect/ocorm'
...
if (metadata) {
  ...
  // 获取所有列
  const allColumns = metadata.getAllColumns()
