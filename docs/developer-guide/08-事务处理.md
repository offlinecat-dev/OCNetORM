...
通过 `Repository.transaction()` 在事务中执行操作（事务成功提交时返回 `TransactionResult`，失败时会抛出 `TransactionRollbackError`）：
...
try {
  ...
  console.log(`事务回滚: ${e instanceof Error ? e.message : String(e)}`)
}
```

### 事务回调

回调函数接收一个 `Repository` 实例作为参数，对当前实体的操作建议通过这个实例执行：
...
如果需要在同一事务中操作其他实体，可以在回调中创建对应实体的 `Repository` 再执行（见下方“多表操作”示例）。
...
事务成功提交时返回 `TransactionResult` 对象；如果发生错误，调用会抛出 `TransactionRollbackError`，不会返回失败的 `TransactionResult`，因此需要使用 `try/catch` 处理：

```typescript
class TransactionResult {
  success: boolean      // 成功提交时为 true
  errorMessage: string  // 失败时通常通过异常抛出，建议在 catch 中读取错误信息
}
```
...
try {
  ...
  console.log(`事务执行失败: ${e instanceof Error ? e.message : String(e)}`)
}
```
...
```typescript
import { Repository, TransactionOptions, IsolationLevel } from '@offlinecat/ocorm'
...
// SERIALIZABLE 快捷方法会在创建时直接抛错（Fail Fast）
try {
  TransactionOptions.serializable()
} catch (e) {
  console.log(`不支持的隔离级别: ${e instanceof Error ? e.message : String(e)}`)
}
```
...
OCORM 的 `IsolationLevel` 枚举如下（运行时支持受 SQLite 能力约束）：

```typescript
enum IsolationLevel {
  READ_UNCOMMITTED = 'READ_UNCOMMITTED',  // 读未提交
  READ_COMMITTED = 'READ_COMMITTED',      // 读已提交（默认）
  REPEATABLE_READ = 'REPEATABLE_READ',    // 当前实现会显式报错
  SERIALIZABLE = 'SERIALIZABLE'           // 当前实现会显式报错
}
```
...
| 隔离级别 | 脏读 | 不可重复读 | 幻读 | 说明 |
|----------|------|------------|------|------|
| READ_UNCOMMITTED | 可能 | 可能 | 可能 | 已支持（`PRAGMA read_uncommitted = 1`） |
| READ_COMMITTED | 不可能 | 可能 | 可能 | 已支持（`PRAGMA read_uncommitted = 0`） |
| REPEATABLE_READ | - | - | - | 当前实现显式拒绝（抛错） |
| SERIALIZABLE | - | - | - | 当前实现显式拒绝（抛错） |
...
> `TransactionOptions.serializable()` 会在创建时抛出错误。  
> 若通过 `fromConfig/new TransactionOptions()` 设置 `SERIALIZABLE` / `REPEATABLE_READ`，会在执行阶段抛出 `TransactionRollbackError`。
...
```typescript
import { Repository, EntityData, TransactionOptions, IsolationLevel } from '@offlinecat/ocorm'
...
async function transfer(fromId: number, toId: number, amount: number) {
  ...
  try {
    await accountRepo.transactionWithOptions(async (txRepo) => {
      // 查询转出账户
...
2. **异常处理**：回调函数中抛出的异常会导致事务回滚，并以 `TransactionRollbackError` 的形式抛出给调用者
