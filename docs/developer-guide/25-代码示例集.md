...
```typescript
// 原生查询：推荐始终使用参数化
async function findLatestActiveUsers(): Promise<Array<EntityData>> {
  return await userRepo.rawQuery(
    'SELECT id, name, email FROM users WHERE is_active = ? ORDER BY id DESC LIMIT 20',
    [1]
  )
}

// 原生写入：仅允许参数化 INSERT/UPDATE/DELETE/REPLACE
async function increaseLoginCount(userId: number): Promise<void> {
...
// 安全闸会拒绝多语句和危险片段
async function unsafeSqlWillFail(): Promise<void> {
  try {
    await userRepo.rawExecute('DELETE FROM users; DROP TABLE users', [])
  } catch (error) {
    console.error(`rawExecute 被拦截: ${error instanceof Error ? error.message : String(error)}`)
  }
}
```
...
```typescript
import { defineEntity, MetadataStorage, ColumnType } from '@offlinecat/ocorm'
...
async function transferPoints(fromUserId: number, toUserId: number, points: number): Promise<void> {
  await userRepo.transaction(async (repo) => {
    ...
    // 扣除积分
    fromUser.setPropertyValue('points', fromPoints - points)
    await repo.save(fromUser)
    
    // 增加积分
    const toPoints = toUser.getPropertyValue('points') as number
    toUser.setPropertyValue('points', toPoints + points)
...
async function reliableOperation(): Promise<void> {
  const options = TransactionOptions.fromConfig({
    ...
    isolation: IsolationLevel.READ_COMMITTED  // 当前版本建议使用已支持级别
  })
  ...
  await userRepo.transactionWithOptions(async (repo) => {
    ...
    if (user) {
      user.setPropertyValue('balance', 100)
...
// 3.0：serializable 为 fail-fast（创建即抛错）
function serializableFailFastExample(): void {
  try {
    TransactionOptions.serializable()
  } catch (error) {
    console.error(`serializable 不可用: ${error instanceof Error ? error.message : String(error)}`)
  }
}

// 3.0：只读事务在不支持 query_only 的环境会明确失败
async function strictReadOnlyFallback(): Promise<void> {
  const options = TransactionOptions.readOnly()
  const result = await userRepo.transactionWithOptions(async (repo) => {
    await repo.findAll()
  }, options)

  if (!result.success) {
    console.error(`只读事务失败（未降级）: ${result.errorMessage}`)
  }
}
```
...
```typescript
import { MetadataStorage, EntityHooks, EntityData } from '@offlinecat/ocorm'
...
```typescript
import { Logger, LogLevel, QueryCache } from '@offlinecat/ocorm'
