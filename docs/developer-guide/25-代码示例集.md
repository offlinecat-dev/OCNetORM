...
```typescript
import { defineEntity, defineRelation, RelationType } from '@aspect/ocorm'
...
// 定义关系
defineRelation('User', 'profile', {
  type: RelationType.ONE_TO_ONE,
  target: 'Profile',
  foreignKey: 'user_id',
  isOwner: false  // 外键在 Profile 表
})

defineRelation('Profile', 'user', {
  type: RelationType.ONE_TO_ONE,
  target: 'User',
  foreignKey: 'user_id',
  isOwner: true   // 外键在本表
...
// 定义关系
defineRelation('User', 'posts', {
  type: RelationType.ONE_TO_MANY,
  target: 'Post',
  foreignKey: 'user_id'
})

defineRelation('Post', 'author', {
...
// 定义多对多关系
defineManyToManyRelation('Article', 'tags', {
  target: 'Tag',
  joinTable: 'article_tags',
  joinSourceKey: 'article_id',
  joinTargetKey: 'tag_id'
})
...
```typescript
import { registerHooks, EntityData } from '@aspect/ocorm'

// 注册钩子
registerHooks('User', {
  // 保存前
  beforeSave: async (data: EntityData) => {
  ...
  // 加载后
  afterLoad: async (data: EntityData) => {
  ...
  // 删除前
  beforeDelete: async (data: EntityData) => {
