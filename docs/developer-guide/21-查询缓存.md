...
- **单例模式**：全局唯一的缓存实例
- **TTL 过期**：支持缓存条目自动过期
- **最旧条目驱逐**：当缓存满时，按写入时间驱逐最旧的条目（不是 LRU）
- **拷贝保护**：属性与 transient 深拷贝；关联数据（related）为浅拷贝（保留 EntityData 引用）
- **统计信息**：提供命中率等统计数据
...
```typescript
import { QueryCache, QueryCacheConfig } from '@offlinecat/ocorm'
...
cache.configure({
  ...
  enabled: true      // 是否启用缓存（常规初始化默认由 DatabaseConfig.enableQueryCache 决定）
})
...
> **提示**：如果你通过 `OCORMInit` / `DatabaseManager.initialize` 初始化数据库，初始化过程会根据 `DatabaseConfig.enableQueryCache`（默认 `false`）重置 `QueryCache` 的启用状态。推荐在初始化前使用 `config.setQueryCache(true, maxSize, ttlMs)` 开启缓存。
...
```typescript
import { QueryCache, EntityData } from '@offlinecat/ocorm'
...
> **注意**：`get` 方法返回的是拷贝后的 `EntityData`。属性与 transient 为深拷贝，但关联数据（related）为浅拷贝（保留引用），修改关联实体对象可能影响缓存副本。
...
```typescript
import { QueryCache, CacheStatistics } from '@offlinecat/ocorm'
...
Repository 默认集成查询缓存；当 `QueryCache` 启用且 `includeDeleted=false` 时，`findById` 会自动读写缓存。
...
// save：
// - 新增（insert）会使该实体所有缓存失效（invalidateEntity）
// - 更新（update）会使该记录缓存失效（invalidate）
await userRepo.save(userData)

// 删除操作（软删/硬删） → 自动使该记录缓存失效（invalidate）
await userRepo.removeById(1)

// 批量插入 → 如果有插入成功，会使该实体所有缓存失效（invalidateEntity）
...
当缓存条目数达到 `maxSize` 时，按写入时间驱逐最旧的条目：
...
缓存使用拷贝机制保护数据完整性（属性/临时数据深拷贝，关联数据浅拷贝）：
...
4. **关联查询**：`Repository.findById` 主要缓存主实体数据；如果手动缓存包含关联，注意 `related` 为浅拷贝引用
...
```typescript
import { Repository, EntityData, QueryCache } from '@offlinecat/ocorm'
