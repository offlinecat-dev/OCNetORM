...
```typescript
import { EntityValidator, ValidationResult } from '@aspect/ocorm'

// 方式一：手动验证
const result: ValidationResult = EntityValidator.validate('User', userData)
if (!result.isValid()) {
  console.error('验证失败:', result.getSummary())
  return
}

// 方式二：自动验证（save 时）
try {
  await userRepo.save(userData)  // 自动执行验证
} catch (error) {
  if (error instanceof ValidationError) {
    console.error('验证失败:', error.message)
  }
...
```typescript
// ✅ 始终验证用户输入
import { registerValidation } from '@aspect/ocorm'
