...
```typescript
import { Logger } from '@offlinecat/ocorm'
...
- `Logger` 默认未启用（`enabled = false`）
- `Logger` 默认日志级别为 `LogLevel.INFO`
- `DatabaseConfig` 默认 `enableLogger = false`，默认 `loggerLevel = LogLevel.INFO`

### 在初始化时配置

```typescript
import { OCORMInit, DatabaseConfig, LogLevel } from '@offlinecat/ocorm'
...
以下示例默认已启用日志（例如先执行 `logger.configure(true, LogLevel.DEBUG)`）。否则 `Logger` 默认关闭，不会输出任何日志。
...
// 输出: [OCORM][QUERY][15ms] SELECT * FROM users WHERE id = 1
...
Logger 会对 `logQuery` 的 SQL 文本进行脱敏处理；`logError` 也会对日志内容做脱敏处理：

### 脱敏规则

1. **SQL 字符串值** - 单引号内的内容替换为 `'[***]'`
2. **长数字** - 超过 6 位的连续数字替换为 `[***]`（可能是手机号、身份证等）
3. **错误日志** - `logError` 会额外对双引号内字符串做脱敏（例如 `"[***]"`）
...
为避免日志过长，消息内容超过 512 字符时会被截断，并在末尾追加 `...`（同时会将控制字符替换为空格）：
