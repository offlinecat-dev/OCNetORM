...
```typescript
import { ValidationMetadataStorage, MetadataStorage } from '@aspect/ocorm'
...
```typescript
import { RequiredValidationError } from '@aspect/ocorm'
...
```typescript
import { LengthValidationError } from '@aspect/ocorm'
...
```typescript
import { EmailValidationError } from '@aspect/ocorm'
...
```typescript
import { EntityValidator, ValidationResult, EntityData } from '@aspect/ocorm'
...
```typescript
import { EntityValidator, EntityValidationError } from '@aspect/ocorm'
...
try {
  await repo.save(userData)  // 验证失败会抛出异常
} catch (error) {
...
2. **空值处理**：`length` 和 `email` 规则对 null/空字符串跳过验证，需配合 `required` 使用
