...
```typescript
import { Repository, BatchInsertOptions } from '@aspect/ocorm'
...
class BatchInsertResult {
  ...
  failedIndexes: number[]    // 失败的索引（如可确定）
...
console.log(`插入数量: ${result.insertedCount}/${result.totalCount}`)
...
if (result.success) {
  ...
  // 部分成功
  console.log(`部分成功: ${result.insertedCount}/${result.totalCount}`)
} else {
  // 全部失败
...
```typescript
import { Repository, EntityData, BatchInsertOptions } from '@aspect/ocorm'
