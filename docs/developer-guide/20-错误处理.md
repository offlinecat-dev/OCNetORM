...
注意：`OrmError` 会根据当前语言环境，优先使用已注册的错误消息模板（按 `code` 查找）生成最终的 `message`；当找不到对应模板时，才会使用构造函数传入的 `message` 作为兜底。需要查看更详细的上下文信息可使用 `getFullMessage()`。

### 使用示例

```typescript
import { Repository, OrmError } from '@offlinecat/ocorm'
...
// 错误码: ORM003, 消息: 列 "profile" 的类型不受支持: 无效类型: CUSTOM_TYPE
...
// 错误码: ORM103, 消息: 不支持的查询操作符: 无效操作符: LIKE_ANY
...
// 错误码: ORM201, 消息: 类型转换失败: 从 string 转换到 number
...
// 错误码: ORM204, 消息: 类型不匹配: 期望类型: number，实际类型: string
```
...
// 错误码: ORM401, 消息: 数据库连接失败: Permission denied

// SQL 执行失败
throw new ExecutionError('INSERT INTO users', 'UNIQUE constraint failed')
// 错误码: ORM402, 消息: SQL 执行失败: SQL: INSERT INTO users... | 原因: UNIQUE constraint failed

// 数据库未初始化
throw new DatabaseNotInitializedError()
// 错误码: ORM403, 消息: 数据库未初始化
```

### 关联关系错误

与实体关联相关的错误（这些错误类直接继承自 `OrmError`）：
...
// 错误码: ORM501, 消息: 实体 "User" 的关联关系未找到: 关联关系 'posts' 未注册

// 无效关系类型
throw new InvalidRelationTypeError('INVALID_TYPE')
// 错误码: ORM502, 消息: 无效的关系类型: 无效的关系类型: INVALID_TYPE

// 外键位置错误
throw new InvalidForeignKeySideError('invalid')
// 错误码: ORM503, 消息: 无效的外键位置: 无效的外键位置: invalid
...
// 错误码: ORM801, 消息: 数据验证失败
```

### 其他错误类型

```typescript
import { HookExecutionError } from '@offlinecat/ocorm'
import { SoftDeleteNotEnabledError } from '@offlinecat/ocorm'
...
```typescript
import { setErrorLocale, getErrorLocale } from '@offlinecat/ocorm'
...
// 获取当前语言
const currentLocale = getErrorLocale()  // 当前 locale（例如 'zh'、'en' 或其他已设置的语言）
```
...
```typescript
import { registerErrorLocaleMessages, ErrorLocaleMessages } from '@offlinecat/ocorm'
...
```typescript
import { registerErrorLocaleMessages, ErrorLocaleMessages, setErrorLocale } from '@offlinecat/ocorm'
...
```typescript
import { OrmError, DatabaseError, ValidationError, QueryError } from '@offlinecat/ocorm'
...
```typescript
import { OrmError, ERROR_DATABASE_NOT_INITIALIZED, ERROR_ENTITY_NOT_REGISTERED } from '@offlinecat/ocorm'
...
```typescript
import { ValidationError, RequiredValidationError, LengthValidationError } from '@offlinecat/ocorm'
...
```typescript
import { DatabaseError, TransactionError } from '@offlinecat/ocorm'
