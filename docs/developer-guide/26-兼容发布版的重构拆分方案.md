...
- ⚠️ `withWhere('commentable', ...)`：MVP 暂不支持，统一抛 `InvalidConditionError`
- ⚠️ `commentable.xxx` 嵌套路径：MVP 暂不支持，统一抛 `RelationNotFoundError`（或等价关系路径错误）
...
- ✅ `src/test`：已覆盖 MorphTo 注册、加载、路径异常与 MVP 限制
- ✅ `entry RelationSuite`：真实关系混合加载（`Comment -> Post/User`）可稳定通过
- ✅ 文档：路线图与开发者指南已同步标注 3.4（9.1）状态
- ✅ `entry BoundarySuite`：已补充未知 type、空 id/type、路径异常与 withWhere 限制回归
