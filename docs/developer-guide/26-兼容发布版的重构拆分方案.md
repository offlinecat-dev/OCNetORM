...
> 状态：9.4 已实现（2026-02-09）
> 目标：在不破坏已发布 API 的前提下新增 MorphTo 能力。

### 9.1 MVP 范围

- ✅ 新增单值多态关联（`MorphTo`），典型场景：`Comment.commentable -> Post | User`
- ✅ 支持 `with('commentable')` 预加载
- ✅ 支持 `withLazy('commentable')` 延迟加载
- ✅ 支持 `withCount('commentable')` 计数（结果为 `0/1`）
- ✅ 支持 `defineEntity(..., morphTo: {...})` 声明式注册
- ✅ 保留手动注册入口，避免强制迁移历史代码

### 9.2 兼容性边界

- 不变更既有 `ONE_TO_ONE/ONE_TO_MANY/MANY_TO_ONE/MANY_TO_MANY` 行为
- 不重命名已发布类与方法
- `Index.ets` 维持向后兼容导出
- 旧业务未声明 `morphTo` 时，运行行为保持不变

### 9.3 拟新增类型与注册 API

- `RelationType` 新增 `MORPH_TO`
- 新增 `MorphToMetadata`：
  - `propertyName`
  - `idColumn`
  - `typeColumn`
  - `typeMap?: Record<string, string>`（可选，类型值映射到实体名）
- `MetadataStorage` 新增：
  - `registerMorphTo(sourceEntity, propertyName, idColumn, typeColumn, typeMap?)`
  - `getMorphToRelation(entityName, propertyName)`
- `EntitySchema` 新增可选配置：
  - `morphTo?: { name: string, idColumn: string, typeColumn: string, typeMap?: Record<string, string> }`

### 9.4 查询执行策略（9.4）

- ✅ `with('commentable')`：已支持
- ✅ `with('commentable.xxx')`：已支持（按动态目标实体分组递归）
- ✅ `withLazy('commentable')`：已支持
- ✅ `withCount('commentable')`：已支持，结果按单值关系计数（`0/1`）
- ✅ `withCount('commentable.xxx')`：已支持（部分类型缺失路径时按 `0` 计数）
- ✅ `withWhere('commentable', ...)`：已支持（按目标实体执行回调过滤）
- ✅ `withWhere('commentable.xxx', ...)`：已支持（动态目标实体分组后在二级关系上执行过滤）

### 9.5 分阶段落地步骤

1. ✅ 元数据层：已新增 `RelationType.MORPH_TO`、`MorphToMetadata`、`MetadataStorage` 注册/查询入口
2. ✅ Schema 层：`defineEntity` 已支持 `morphTo` 声明并注册元数据
3. ✅ 执行层：`RelationLoader` 已增加 `loadMorphTo`，并补充路径防御与计数支持
4. ✅ 验证层：`src/test`、`entry RelationSuite` 与 `entry BoundarySuite` 已补齐

### 9.6 验证清单（通过标准）

- ✅ `src/test`：已覆盖 MorphTo 注册、加载、嵌套路径与 withWhere 行为
- ✅ `entry RelationSuite`：真实关系混合加载（`Comment -> Post/User`）可稳定通过
- ✅ 文档：路线图与开发者指南已同步标注 3.4（9.4）状态
- ✅ `entry BoundarySuite`：已补充未知 type、空 id/type、非法嵌套路径与 withWhere 异常回归
