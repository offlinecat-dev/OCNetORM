...
// 注册关系
MetadataStorage.getInstance().addRelation(userToPosts)
```
...
MetadataStorage.getInstance().addRelation(postToUser)
```
...
if (user !== null) {
  // 获取关联的文章列表
  const posts = user.getRelations('posts')  // 返回 EntityData[]
...
for (const post of posts) {
  const author = post.getRelation('author')  // 单个实体用 getRelation
...
for (const user of users) {
  const posts = user.getRelations('posts')
...
async function example() {
  ...
  if (userWithPosts !== null) {
    const posts = userWithPosts.getRelations('posts')
...
4. **空集合处理**：`getRelations` 返回的可能是空数组，使用前检查
