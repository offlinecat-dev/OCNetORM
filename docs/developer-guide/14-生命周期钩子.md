...
```typescript
import { defineEntity, ColumnType, EntityData } from '@aspect/ocorm'
...
defineEntity('User', {
  ...
  hooks: {
    ...
    afterLoad: (data: EntityData) => {
      // 加载后的数据处理
      console.log(`加载了用户: ${data.getPropertyValue('name')}`)
    },
    beforeDelete: (data: EntityData) => {
      // 删除前的检查
...
interface EntityHooks {
  beforeSave?: HookCallback | null   // 保存前
  afterLoad?: HookCallback | null    // 加载后
  beforeDelete?: HookCallback | null // 删除前
}
```
...
try {
  ...
  if (error instanceof HookExecutionError) {
    console.log(`钩子执行失败:`)
    console.log(`  实体: ${error.entityName}`)
    console.log(`  钩子: ${error.hookName}`)
