...
import { EntityDataInput } from '@aspect/ocorm'
...
`DataMapper` 处理 `EntityData` 与数据库记录（`ValuesBucket`、`ResultSet`）之间的双向转换。

### 创建 DataMapper

```typescript
import { DataMapper } from '@aspect/ocorm'
...
```typescript
import { DataMapper, ResultSetRow } from '@aspect/ocorm'
...
|------------|------------|----------|
| string | TEXT | 直接存储 |
| number | INTEGER/REAL | 整数存 INTEGER，浮点存 REAL |
| boolean | INTEGER | true → 1, false → 0 |
| Date | INTEGER | 时间戳（毫秒） |
| object | TEXT | JSON 序列化 |
| Uint8Array | BLOB | 二进制数据 |

### 基础类型转换

```typescript
import { TypeConverter } from '@aspect/ocorm'
...
```typescript
import { TypeConverter, ColumnType } from '@aspect/ocorm'
...
```typescript
import { ValuesBucket } from '@aspect/ocorm'
...
```typescript
import { ResultSetRow } from '@aspect/ocorm'
...
```typescript
import { EntityDataInput, EntityData } from '@aspect/ocorm'
...
```typescript
import { RelatedDataValue, EntityData } from '@aspect/ocorm'
...
```typescript
import { Repository, EntityData, EntityDataInput, DataMapper } from '@aspect/ocorm'
...
3. **属性类型标识** - `addProperty` 的第三个参数用于类型转换，需正确设置
