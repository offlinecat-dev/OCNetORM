...
## 1. 目标
- 说明 `事务执行层` 在当前仓库中的真实事务执行路径。
- 说明普通事务与带选项事务在超时、重试、只读、隔离级别上的行为差异。
- 明确哪些失败会直接抛 `TransactionRollbackError`，哪些会先进入后台清理。

## 2. 背景与适用范围
`Repository.transaction(...)` 和 `Repository.transactionWithOptions(...)` 最终都落到 `事务执行层`。这一层负责：
- 获取 `StoreLease`
- 给连接加事务锁
...
如果你只是想在业务代码里开事务，应优先通过 `Repository` 调用，而不是手动 new `事务执行层`。
...
Repository.transaction(...)
  -> 事务执行层.acquireTransactionLeaseOrThrow()
  -> 事务执行层.acquireTransactionLock(store)
