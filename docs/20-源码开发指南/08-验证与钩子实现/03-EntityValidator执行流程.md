...
## 1. 目标
`EntityValidator` 的职责不是注册规则，而是消费 `ValidationMetadataStorage` 中已经登记的规则，对单个实体数据做一次完整扫描，并输出 `ValidationResult` 或抛出 `ValidationError`。
...
不适用于以下问题：
- 规则如何声明：看 `ValidationDecorators.ets`
- 钩子何时触发：看 `HooksProcessor.ets`

## 3. 前置条件
- 规则已经通过 `ValidationDecorators` 注册进 `ValidationMetadataStorage`
- 调用方已经准备好实体名与实体数据对象
...
validate(entityName, entityData, options)
  -> ValidationMetadataStorage.getRules(entityName)
