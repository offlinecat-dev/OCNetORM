...
### 2.3 测试桥接入口迁移到 `DatabaseManagerTestHooks`
当前仓库里测试注入能力已经转移到 `database/testing/DatabaseManagerTestHooks`，旧公开面不应再被继续依赖。

```arkts
import { DatabaseManagerTestHooks } from 'ocorm'
...
- [ ] 测试桥接已迁移到 `DatabaseManagerTestHooks`
- [ ] 事务回调里统一复用 `txRepo`
- [ ] 未直接依赖内部 support 模块或 `rawsql/*` 作为对外 API

```bash
rg -n "rawQuery\(|rawQuerySafe\(|rawExecute\(|selectRaw\(|groupBy\(|having\(|DatabaseManagerTestHooks|serializable\(|transaction\(" OCORM entry
```
