...
> 本文档列出 OCORM 未来版本可增强或新增的功能建议。所有新功能均向后兼容，不影响现有 API。
>
> 适配主版本：**3.0.0**
...
- 本路线图涉及的功能增强与回归测试，统一写在 `entry/src/main/ets` 目录（推荐放在 `entry/src/main/ets/suites`）。
- `OCORM/src/test` 保留为库级单元测试目录，不作为本路线图增量功能的默认验收落点。
...
| 模块 | 已实现功能 |
|------|-----------|
| **实体定义** | defineEntity、装饰器（@Table、@Column、@PrimaryKey、@CreatedAt、@UpdatedAt、@SoftDelete）、类型推断 |
| **CRUD** | save、findById、findAll、findAllAsync、remove、forceRemove、restore、count |
| **查询构建器** | where/andWhere/orWhere、whereIn/whereNull/whereLike/whereBetween、whereExists（子查询）、orderBy、limit/offset、select、paginate、chunk、timeout |
| **关联关系** | OneToOne、OneToMany、ManyToMany、级联操作、attach/detach/sync、with（预加载）、withLazy（延迟加载）、withWhere（关联过滤）、withCount（关联计数） |
| **事务** | transaction、transactionWithOptions（超时/重试/隔离级别/只读） |
| **批量操作** | batchInsert、saveAll |
| **软删除** | withDeleted、onlyDeleted、restore |
| **验证** | @Required、@Length、@Email |
| **迁移** | MigrationManager、autoMigrate、版本控制、SchemaDiffer |
| **缓存** | QueryCache（一级缓存、TTL、LRU、统计） |
| **日志** | Logger（多级别日志、查询日志、事务日志） |
| **钩子** | beforeSave、afterLoad、beforeDelete |
...
#### 2.2 条件验证 ✅ 已实现
```typescript
@RequiredIf('type', 'premium')  // 条件必填
@RequiredUnless('role', 'admin') // 条件非必填
```
...
// 加载多层关联
await repo.createQueryBuilder()
  .with('posts.comments.author')  // 嵌套加载
  .with('profile')
  .get()
```
...
// 结果: { id: 1, name: 'John', posts_count: 10, comments_count: 25 }
```
...
> 说明：
> - 非法嵌套路径（如 `commentable.notExistRelation`）在执行阶段抛出 `ExecutionError`
...
- 状态：✅ 已完成（2026-02-10，entry 模块 ETS 测试通过）
...
#### 5.1 更多生命周期钩子
```typescript
// 现有钩子: beforeSave, afterLoad, beforeDelete
// 建议新增钩子:
beforeInsert   // 仅插入前（区分于 beforeSave）
afterInsert    // 仅插入后
beforeUpdate   // 仅更新前（区分于 beforeSave）
afterUpdate    // 仅更新后
afterDelete    // 删除后
afterSave      // 保存后（插入或更新后）
beforeRestore  // 恢复前（软删除）
afterRestore   // 恢复后（软删除）
```
...
- 状态：✅ 已完成（2026-02-10，5.1~5.4 已在 OCORM 主代码落地，并由 DeveloperExperienceSuite 覆盖）
...
defineEntity({
  ...
  indexes: [
    ...
    { columns: ['status', 'created_at'] }  // 复合索引
  ]
})
```
...
#### 6.2 唯一约束验证
```typescript
// 保存前自动检查唯一性（应用层验证，非数据库约束）
@Unique()  // 装饰器方式
// 或
{ name: 'email', type: ColumnType.TEXT, unique: true }  // Schema 方式
```

- 状态：✅ 已完成（2026-02-10，6.1 索引元数据+自动同步已实现；6.2 唯一约束验证已实现）
...
- 状态：✅ 已完成（2026-02-10，Seeder/Factory/MigrationGenerator 已实现并导出）

---

## 📊 实现优先级矩阵

| 功能 | 用户价值 | 实现复杂度 | 建议优先级 | 状态 |
|------|---------|-----------|-----------|------|
| 聚合函数(sum/avg/max/min) | ⭐⭐⭐⭐⭐ | ⭐⭐ | P0 | ✅ 已完成 |
| 更多验证器 | ⭐⭐⭐⭐⭐ | ⭐⭐ | P0 | ✅ 已完成 |
| 分组查询 | ⭐⭐⭐⭐ | ⭐⭐ | P1 | ✅ 已完成 |
| 原生 SQL | ⭐⭐⭐⭐ | ⭐ | P1 | ✅ 已完成 |
| 批量更新/删除 | ⭐⭐⭐⭐ | ⭐⭐ | P1 | ✅ 已完成 |
...
| 更多钩子 | ⭐⭐⭐ | ⭐⭐ | P2 | 待实现 |
| 关联计数 | ⭐⭐⭐ | ⭐⭐ | P2 | ✅ 已完成 |
| 多态关联（MorphTo） | ⭐⭐⭐ | ⭐⭐⭐ | P2 | ✅ 9.4 已完成（with/withLazy/withCount/withWhere/嵌套） |
| 分块查询 | ⭐⭐⭐ | ⭐⭐ | P2 | ✅ 已完成 |
| toSQL/explain | ⭐⭐ | ⭐ | P3 | 待实现 |
| 数据工厂 | ⭐⭐ | ⭐⭐ | P3 | ✅ 已完成 |
...
### v1.1.0 - 查询增强版
- [x] 聚合函数（sum/avg/max/min） ✅
- [x] 分组查询（groupBy/having/selectRaw） ✅
- [x] 原生 SQL 查询（rawQuery/rawExecute） ✅
- [x] 查询作用域（scope/scopes） ✅
- [x] 批量更新/删除 ✅

### v1.2.0 - 验证增强版
- [x] 新增验证装饰器（@Min/@Max/@Range/@Pattern 等） ✅
- [x] 条件验证 ✅
- [x] 验证分组 ✅
- [x] 自定义验证函数 ✅
...
### v2.0.0 - 扩展特性版
- [x] 多态关联（9.4：with/withLazy/withCount/withWhere/嵌套加载/声明注册） ✅
- [ ] 事件系统
- [x] 数据工厂/填充器 ✅
...
1. **新增 API 不影响现有 API** - 现有代码无需修改即可升级
2. **可选参数使用默认值** - 新参数均有合理默认值
3. **废弃而非删除** - 旧 API 标记 @deprecated 但保留至少两个大版本
4. **配置驱动** - 新功能可通过配置开关控制
